...
mod filters {
    ...
    use feature_flags::db::{DBLite, Flag, FlagValue, KeyOnly};
    ...
    fn json_key_body() -> impl Filter<Extract = (KeyOnly,), Error = warp::Rejection> + Clone {
        warp::body::content_length_limit(1024 * 16).and(warp::body::json())
    }
}
...
mod handlers {
        ...
        if new_flag.key != env!("SEC_KEY") {
            return Ok(warp::reply::with_status(
                warp::reply::json(&ResponseMessage {
                    ...
                    message: "Key provided is not correct.".to_string(),
                }),
    ...
    pub async fn update_flag(
        ...
        if !exist {
            return Ok(warp::reply::with_status(
                warp::reply::json(&ResponseMessage {
                    ...
                    message: "Flag was not found.".to_string(),
                }),
        ...
        match result {
            ...
            Err(err) => {
                log::debug!("Unable to update flag");
                Ok(warp::reply::with_status(
                    warp::reply::json(&ResponseMessage {
                        code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                        message: format!("Unable to update key, {:?}", err.to_string()),
                    }),
        ...
        match result {
            ...
            Err(err) => {
                ...
                Ok(warp::reply::with_status(
                    warp::reply::json(&ResponseMessage {
                        code: StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
                        message: format!("Unable to delete key, {:?}", err.to_string()),
                    }),
                    StatusCode::INTERNAL_SERVER_ERROR,
                ))
            }
